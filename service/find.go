...
func InsertMsg2Mongo(database, id string, content string, read uint, expire int64) (err error) {

	db := conf.MongoDBClient.Database(database)
	collection := db.Collection(id)  // 没有id这个集合，会自动创建
...
func FindMany(database string, sendId string, id string, queryTime int64, pageSize int) (results []model.Result, err error)  {
	...
	err = sendIdTimeCursor.All(context.TODO(), &resultsMe)
	err = idTimeCursor.All(context.TODO(), &resultsYou)
	results, _ = AppendAndSort(resultsMe, resultsYou)
	return
}

func FirsFindMsg(database string,sendId string,id string) (results []model.Result,err error) {
	// 首次查询(把对方发来的所有未读都取出来)
	var resultsMe []model.Trainer
	var resultsYou []model.Trainer
	sendIdCollection := conf.MongoDBClient.Database(database).Collection(sendId)
